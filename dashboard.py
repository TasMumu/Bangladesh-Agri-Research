# ═══════════════════════════════════════════════════════════════════
# Bangladesh Agricultural Vulnerability Dashboard
# Climate-Driven Vulnerability of Boro Rice Yield · 64 Districts · 2001–2022
# Author: Tasnim Ahmad Mumu
# Run: streamlit run dashboard.py
# ═══════════════════════════════════════════════════════════════════

import streamlit as st
import pandas as pd
import numpy as np
import geopandas as gpd
import plotly.express as px
import plotly.graph_objects as go
import duckdb
from pathlib import Path

# ── Page config ────────────────────────────────────────────────────
st.set_page_config(
    page_title="Bangladesh Agricultural Vulnerability",
    page_icon="🌾",
    layout="wide",
    initial_sidebar_state="expanded",
)

# ── CSS ────────────────────────────────────────────────────────────
# Design principles:
#   • Sidebar always dark (consistent brand regardless of theme)
#   • All Plotly legends use a semi-transparent neutral bg with
#     explicit dark text so they are readable in BOTH light & dark mode
#   • Grid lines use a neutral rgba so they work in both themes
#   • No hardcoded white/black text in the main content area
# ──────────────────────────────────────────────────────────────────
st.markdown("""
<style>
/* ── Sidebar ──────────────────────────────────────────────────── */
[data-testid="stSidebar"] {
    background: #0d1117 !important;
    border-right: 1px solid #21262d;
}
[data-testid="stSidebar"] * { color: #c9d1d9 !important; }
[data-testid="stSidebar"] hr { border-color: #21262d !important; }
[data-testid="stSidebar"] [data-testid="stRadio"] label {
    font-size: 0.875rem !important;
    padding: 7px 12px !important;
    border-radius: 6px !important;
    transition: background 0.15s;
    color: #c9d1d9 !important;
}
[data-testid="stSidebar"] [data-testid="stRadio"] label:hover {
    background: #161b22 !important;
}
[data-testid="stSidebar"] .stCaption {
    color: #484f58 !important;
    font-size: 0.7rem !important;
}

/* ── Sidebar logo ─────────────────────────────────────────────── */
.sb-logo {
    display: flex; align-items: center; gap: 12px;
    padding: 4px 0 14px 0;
    border-bottom: 1px solid #21262d;
    margin-bottom: 16px;
}
.sb-logo .icon { font-size: 1.8rem; line-height: 1; }
.sb-logo .title { font-size: 0.92rem; font-weight: 700;
                  color: #f0f6fc !important; letter-spacing: -0.02em; margin: 0; }
.sb-logo .sub   { font-size: 0.70rem; color: #484f58 !important;
                  margin: 2px 0 0; line-height: 1.3; }
.nav-lbl {
    font-size: 0.62rem; font-weight: 700; letter-spacing: 0.12em;
    text-transform: uppercase; color: #484f58 !important;
    margin: 8px 0 4px 2px;
}

/* ── Metric cards ─────────────────────────────────────────────── */
[data-testid="stMetric"] {
    border: 1px solid rgba(48,54,61,0.6);
    border-radius: 10px;
    padding: 14px 18px;
    background: rgba(22,197,94,0.04);
}
[data-testid="stMetricLabel"] {
    font-size: 0.70rem !important;
    font-weight: 700 !important;
    text-transform: uppercase !important;
    letter-spacing: 0.07em !important;
    color: #16c55e !important;
}
[data-testid="stMetricValue"] {
    font-size: 1.5rem !important;
    font-weight: 700 !important;
}

/* ── Page title accent ────────────────────────────────────────── */
h1 { border-left: 4px solid #16c55e; padding-left: 14px; }

/* ── Tab styling ──────────────────────────────────────────────── */
[data-testid="stTab"] { font-weight: 600; }

/* ── Vulnerability card text always readable ──────────────────── */
.vuln-card p, .vuln-card b { color: inherit !important; }
</style>
""", unsafe_allow_html=True)

# ── Paths — edit ROOT to match your machine ────────────────────────
ROOT   = Path(r"E:\Tasnim Docs\Projects\GIS Projects\GIS Project\BD_Rice_Climate\bangladesh_agri")
SILVER = ROOT / "data/silver"
GOLD   = ROOT / "data/gold"

# ── Plotly legend style ────────────────────────────────────────────
# Applied consistently to every chart that has a legend.
# Semi-transparent near-black bg with white text works in both themes.
LEGEND = dict(
    bgcolor="rgba(15,17,23,0.82)",
    font=dict(color="#e2e8f0", size=12),
    bordercolor="rgba(255,255,255,0.12)",
    borderwidth=1,
)

# Neutral grid colour that works in both light and dark theme
GRID = "rgba(120,120,120,0.15)"

# Transparent plot/paper background — let Streamlit theme show through
PLOT_BG = dict(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")


# ── Data loading ───────────────────────────────────────────────────
@st.cache_data
def load_vulnerability():
    con = duckdb.connect(str(GOLD / "bangladesh_agri.duckdb"), read_only=True)
    df  = con.execute("SELECT * FROM mart_vulnerability_index").df()
    con.close()
    return df

@st.cache_data
def load_panel():
    con = duckdb.connect(str(GOLD / "bangladesh_agri.duckdb"), read_only=True)
    df  = con.execute("""
        SELECT district_name, division_name, year, season,
               boro_yield_mt_ha AS yield_mt_ha,
               ndvi, ndvi_anomaly,
               precip_total_mm, precip_anomaly_mm,
               temp_mean_c, temp_anomaly_c,
               flood_fraction, is_major_flood_year,
               flood_risk_tier, yield_data_source
        FROM fact_district_season_v2
        WHERE season = 'boro' AND boro_yield_mt_ha IS NOT NULL
        ORDER BY district_name, year
    """).df()
    con.close()
    return df

@st.cache_data
def load_geodata():
    candidates = [
        ROOT / "data/bronze/gadm_shapefiles/bangladesh_districts_clean.shp",
        ROOT / "data/bronze/gadm_shapefiles/gadm41_BGD_2.shp",
    ]
    shp = next((p for p in candidates if p.exists()), None)
    if shp is None:
        return None
    gdf = gpd.read_file(shp).rename(columns={
        "district_n":"district_name","division_n":"division_name",
        "NAME_2":"district_name","NAME_1":"division_name",
    }).to_crs("EPSG:4326")
    return gdf[[c for c in ["district_name","division_name","geometry"] if c in gdf.columns]]

@st.cache_data
def load_spatial_analysis():
    p = SILVER / "silver_spatial_analysis.parquet"
    return gpd.read_parquet(p) if p.exists() else None

@st.cache_data
def load_slm_results():
    """Load spatial lag model results added in the econometric notebook."""
    p = ROOT / "outputs/analysis/spatial_lag_model_results.csv"
    return pd.read_csv(p) if p.exists() else None

df_vuln  = load_vulnerability()
df_panel = load_panel()
gdf      = load_geodata()
gdf_lisa = load_spatial_analysis()
df_slm   = load_slm_results()

# ── Pre-compute geo objects once ───────────────────────────────────
@st.cache_data
def build_vuln_map(_gdf, _df_vuln):
    if _gdf is None:
        return None, None
    merged  = _gdf.drop(columns=["division_name"], errors="ignore").merge(
                  _df_vuln, on="district_name", how="left")
    merged["lon"] = merged.geometry.centroid.x
    merged["lat"] = merged.geometry.centroid.y
    return merged.drop(columns="geometry"), merged.__geo_interface__

@st.cache_data
def build_lisa_map(_gdf_lisa):
    if _gdf_lisa is None:
        return None, None
    g = _gdf_lisa.copy()
    if g.crs is None:
        g = g.set_crs("EPSG:3106")
    g = g.to_crs("EPSG:4326")
    g["cluster"] = g.apply(
        lambda r: r["lisa_label"] if r["lisa_sig"] == 1 else "Not significant", axis=1)
    return g.drop(columns="geometry"), g.__geo_interface__

vuln_df_map, vuln_geojson = build_vuln_map(gdf, df_vuln)
lisa_df_map, lisa_geojson = build_lisa_map(gdf_lisa)

ALL_DIVS  = sorted(df_panel["division_name"].dropna().unique())
ALL_TIERS = ["High", "Medium", "Low"]
YEAR_MIN, YEAR_MAX = int(df_panel["year"].min()), int(df_panel["year"].max())

TIER_COLORS = {"High":"#993C1D","Medium":"#C47B2B","Low":"#3B6D11"}
TIER_EMOJI  = {"High":"🔴","Medium":"🟡","Low":"🟢"}

# ── Sidebar ────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("""
<div class="sb-logo">
  <div class="icon">🌾</div>
  <div>
    <p class="title">BD Agri Vulnerability</p>
    <p class="sub">Boro Rice · 64 Districts · 2001–2022</p>
  </div>
</div>
""", unsafe_allow_html=True)

    st.markdown('<p class="nav-lbl">Navigation</p>', unsafe_allow_html=True)
    page = st.radio(
        "nav", ["Overview","District Explorer","Vulnerability Map",
                "Flood & Climate","Regression Findings","About"],
        label_visibility="collapsed",
    )
    st.divider()
    st.caption("Sources: BBS · FAO · MODIS · ERA5 · BARC · JRC · GADM")
    st.caption("Moran's I = 0.804 (p < 0.001)")
    st.caption("Two-way FE panel regression + Spatial Lag Model")


# ══════════════════════════════════════════════════════════════════
# PAGE 1 — OVERVIEW
# ══════════════════════════════════════════════════════════════════
if page == "Overview":
    st.title("Bangladesh Boro Rice — Vulnerability Dashboard")
    st.markdown(
        "Multi-source spatial research project analysing climate-driven vulnerability "
        "of Boro rice yield across **64 districts** from **2001 to 2022**.  "
        "Pipeline: Bronze → Silver → Gold (DuckDB) · Spatial: Queen contiguity Moran's I "
        "· Econometric: Two-way FE panel regression + Spatial Lag Model."
    )

    with st.expander("🔽  Filters", expanded=False):
        oc1, oc2 = st.columns(2)
        with oc1:
            sel_div_ov  = st.multiselect("Division", ALL_DIVS,  default=[], key="ov_div",  placeholder="All divisions")
        with oc2:
            sel_tier_ov = st.multiselect("Tier",     ALL_TIERS, default=[], key="ov_tier", placeholder="All tiers")

    dvf = df_vuln.copy()
    dpf = df_panel.copy()
    if sel_div_ov:
        dvf = dvf[dvf["division_name"].isin(sel_div_ov)]
        dpf = dpf[dpf["division_name"].isin(sel_div_ov)]
    if sel_tier_ov:
        dvf = dvf[dvf["vulnerability_tier"].isin(sel_tier_ov)]
        dpf = dpf[dpf["district_name"].isin(dvf["district_name"])]

    c1,c2,c3,c4,c5 = st.columns(5)
    c1.metric("Districts shown",    len(dvf))
    c2.metric("Study period",       "2001–2022")
    c3.metric("High vulnerability", f"{len(dvf[dvf['vulnerability_tier']=='High'])} districts")
    c4.metric("Mean Boro yield",    f"{dpf['yield_mt_ha'].mean():.2f} MT/ha" if len(dpf) else "—")
    c5.metric("Moran's I",          "0.804***")

    st.divider()
    col_a, col_b = st.columns(2)

    with col_a:
        st.subheader("Vulnerability tier distribution")
        tc = dvf["vulnerability_tier"].value_counts().reindex(ALL_TIERS, fill_value=0).reset_index()
        tc.columns = ["Tier","Count"]
        fig = px.bar(tc, x="Tier", y="Count", color="Tier",
                     color_discrete_map=TIER_COLORS, text="Count")
        fig.update_traces(textposition="outside", marker_line_width=0)
        fig.update_layout(showlegend=False, height=300,
                          margin=dict(t=10,b=10,l=10,r=10),
                          **PLOT_BG,
                          yaxis=dict(showgrid=True, gridcolor=GRID))
        st.plotly_chart(fig, use_container_width=True)

    with col_b:
        st.subheader("National mean Boro yield (2001–2022)")
        yr = dpf.groupby("year")["yield_mt_ha"].mean().reset_index()
        fig = px.area(yr, x="year", y="yield_mt_ha", markers=True,
                      labels={"yield_mt_ha":"Mean yield (MT/ha)","year":"Year"},
                      color_discrete_sequence=["#2e7d32"])
        fig.update_traces(fill="tozeroy", fillcolor="rgba(46,125,50,0.12)")
        for yr_flood in [2004,2007,2017]:
            fig.add_vline(x=yr_flood, line_dash="dash", line_color="#993C1D", opacity=0.6,
                          annotation_text=str(yr_flood), annotation_position="top",
                          annotation_font_size=11, annotation_font_color="#993C1D")
        fig.update_layout(height=300, margin=dict(t=10,b=10,l=10,r=10),
                          **PLOT_BG,
                          yaxis=dict(showgrid=True, gridcolor=GRID))
        st.plotly_chart(fig, use_container_width=True)

    st.divider()
    st.subheader("Most vulnerable districts")
    n_show = st.slider("Show top N", 5, 30, 10, key="ov_topn")
    top_n  = dvf.nlargest(n_show, "vulnerability_index")[
        ["district_name","division_name","vulnerability_index",
         "vulnerability_tier","n_poor_ndvi_years","mean_ndvi_boro"]
    ].reset_index(drop=True)
    top_n.index += 1
    top_n.columns = ["District","Division","Vuln. Index","Tier","Poor NDVI yrs","Mean NDVI"]
    st.dataframe(
        top_n.style
        .background_gradient(subset=["Vuln. Index"], cmap="RdYlGn_r", vmin=0, vmax=1)
        .format({"Vuln. Index":"{:.3f}","Mean NDVI":"{:.3f}"}),
        use_container_width=True,
        height=min(40*n_show+60, 600),
    )


# ══════════════════════════════════════════════════════════════════
# PAGE 2 — DISTRICT EXPLORER
# ══════════════════════════════════════════════════════════════════
elif page == "District Explorer":
    st.title("District Explorer")
    st.markdown("Drill into any district's 22-year yield, NDVI, climate, and flood record.")

    dc1, dc2, dc3 = st.columns([1,2,2])
    with dc1:
        sel_div = st.selectbox("Division", ["All"]+list(ALL_DIVS), key="de_div")
    with dc2:
        opts = (sorted(df_panel[df_panel["division_name"]==sel_div]["district_name"].unique())
                if sel_div != "All"
                else sorted(df_panel["district_name"].unique()))
        sel_dist = st.selectbox("District", opts, key="de_dist")
    with dc3:
        yr_rng = st.slider("Year range", YEAR_MIN, YEAR_MAX,
                           (YEAR_MIN, YEAR_MAX), key="de_yr")

    st.divider()
    left, right = st.columns([1,3])

    with left:
        vrow = df_vuln[df_vuln["district_name"]==sel_dist]
        if len(vrow):
            r     = vrow.iloc[0]
            tier  = r["vulnerability_tier"]
            color = TIER_COLORS.get(tier,"#888")
            emoji = TIER_EMOJI.get(tier,"⚪")
            # Use CSS variable-aware text colours for theme compatibility
            st.markdown(f"""
<div class="vuln-card" style="background:{color}15;border:2px solid {color};
     border-radius:12px;padding:16px;color:inherit;">
  <h4 style="color:{color};margin:0 0 10px 0">{emoji} {sel_dist}</h4>
  <p style="margin:4px 0"><b>Division:</b> {r['division_name']}</p>
  <p style="margin:6px 0"><b>Vulnerability index:</b><br>
    <span style="color:{color};font-size:1.35rem;font-weight:700">
      {r['vulnerability_index']:.3f}
    </span>
    <span style="font-size:0.85rem;opacity:0.7"> / 1.000</span>
  </p>
  <p style="margin:4px 0"><b>Tier:</b> {tier}</p>
  <p style="margin:4px 0"><b>Poor NDVI years:</b> {int(r['n_poor_ndvi_years'])}</p>
  <p style="margin:4px 0"><b>Mean Boro NDVI:</b> {r['mean_ndvi_boro']:.3f}</p>
  <p style="margin:4px 0"><b>Flood risk (BARC):</b> {r.get('flood_risk_tier','—')}</p>
</div>""", unsafe_allow_html=True)

    with right:
        d = (df_panel[(df_panel["district_name"]==sel_dist) &
                      (df_panel["year"]>=yr_rng[0]) &
                      (df_panel["year"]<=yr_rng[1])]
             .sort_values("year"))

        tab1, tab2, tab3 = st.tabs(["📈  Yield & NDVI","🌡️  Climate","🌊  Flood"])

        with tab1:
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=d["year"], y=d["yield_mt_ha"],
                name="Boro yield (MT/ha)",
                line=dict(color="#2e7d32", width=2.5),
                mode="lines+markers", marker=dict(size=7),
            ))
            fig.add_trace(go.Scatter(
                x=d["year"], y=d["ndvi"],
                name="NDVI",
                line=dict(color="#1565c0", width=2, dash="dot"),
                mode="lines+markers", marker=dict(size=6),
                yaxis="y2",
            ))
            for _, row in d[d["is_major_flood_year"]==1].iterrows():
                fig.add_vrect(x0=row["year"]-0.4, x1=row["year"]+0.4,
                              fillcolor="#993C1D", opacity=0.10, line_width=0,
                              annotation_text="flood", annotation_position="top left",
                              annotation_font_size=9, annotation_font_color="#993C1D")
            fig.update_layout(
                title=f"{sel_dist} — Yield & NDVI ({yr_rng[0]}–{yr_rng[1]})",
                yaxis=dict(title=dict(text="Yield (MT/ha)",font=dict(color="#2e7d32")),
                           tickfont=dict(color="#2e7d32"), showgrid=True, gridcolor=GRID),
                yaxis2=dict(title=dict(text="NDVI",font=dict(color="#1565c0")),
                            tickfont=dict(color="#1565c0"),overlaying="y",side="right"),
                legend=LEGEND,
                height=380, margin=dict(t=40,b=20,l=20,r=20),
                **PLOT_BG,
                xaxis=dict(showgrid=True,gridcolor=GRID),
            )
            st.plotly_chart(fig, use_container_width=True)
            if "yield_data_source" in d.columns:
                src = d["yield_data_source"].value_counts().to_dict()
                st.caption(f"Yield data sources: {src}")

        with tab2:
            fig = go.Figure()
            fig.add_trace(go.Bar(
                x=d["year"], y=d["precip_total_mm"],
                name="Precipitation (mm)",
                marker_color="#42a5f5", opacity=0.75,
            ))
            fig.add_trace(go.Scatter(
                x=d["year"], y=d["temp_mean_c"],
                name="Mean temp (°C)",
                line=dict(color="#e53935", width=2.5),
                mode="lines+markers", marker=dict(size=6),
                yaxis="y2",
            ))
            fig.update_layout(
                title=f"{sel_dist} — Climate ({yr_rng[0]}–{yr_rng[1]})",
                yaxis=dict(title=dict(text="Precipitation (mm)",font=dict(color="#1565c0")),
                           tickfont=dict(color="#1565c0")),
                yaxis2=dict(title=dict(text="Temperature (°C)",font=dict(color="#c62828")),
                            tickfont=dict(color="#c62828"),overlaying="y",side="right"),
                legend=LEGEND,
                height=380, margin=dict(t=40,b=20,l=20,r=20),
                **PLOT_BG,
                xaxis=dict(showgrid=True,gridcolor=GRID),
            )
            st.plotly_chart(fig, use_container_width=True)

        with tab3:
            if "flood_fraction" in d.columns:
                flood_color = d["is_major_flood_year"].map({0:"Normal year",1:"Major flood year"})
                fig = px.bar(d, x="year", y="flood_fraction",
                             color=flood_color,
                             color_discrete_map={"Normal year":"#5ba3d9","Major flood year":"#993C1D"},
                             labels={"flood_fraction":"Flood fraction","color":""},
                             title=f"{sel_dist} — Flood fraction ({yr_rng[0]}–{yr_rng[1]})")
                fig.update_layout(height=380, margin=dict(t=40,b=20,l=20,r=20),
                                  legend=LEGEND, **PLOT_BG)
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("Flood data not available.")


# ══════════════════════════════════════════════════════════════════
# PAGE 3 — VULNERABILITY MAP
# ══════════════════════════════════════════════════════════════════
elif page == "Vulnerability Map":
    st.title("Vulnerability & Spatial Clusters")

    tab1, tab2 = st.tabs(["🗺️  Vulnerability Index","🔴  LISA Clusters"])

    with tab1:
        if vuln_df_map is not None:
            mc1, mc2 = st.columns([2,1])
            with mc1:
                sel_tier_map = st.multiselect("Highlight tiers", ALL_TIERS,
                                              default=[], key="map_tier",
                                              placeholder="All tiers shown")
            with mc2:
                map_style = st.selectbox("Map style",
                    ["carto-positron","carto-darkmatter","open-street-map"],
                    key="map_style")

            plot_df = vuln_df_map.copy()
            if sel_tier_map:
                plot_df = plot_df[plot_df["vulnerability_tier"].isin(sel_tier_map)]

            fig = px.choropleth_mapbox(
                plot_df, geojson=vuln_geojson, locations=plot_df.index,
                color="vulnerability_index",
                color_continuous_scale="RdYlGn_r", range_color=(0,1),
                hover_name="district_name",
                hover_data={"division_name":True,"vulnerability_index":":.3f",
                            "vulnerability_tier":True,"n_poor_ndvi_years":True,
                            "mean_ndvi_boro":":.3f"},
                mapbox_style=map_style, zoom=6,
                center={"lat":23.7,"lon":90.4}, opacity=0.75,
                labels={"vulnerability_index":"Vulnerability Index"},
            )
            fig.update_layout(
                height=620, margin=dict(t=0,b=0,l=0,r=0),
                coloraxis_colorbar=dict(
                    title="Vulnerability<br>Index",
                    tickvals=[0,0.25,0.5,0.75,1],
                    ticktext=["0 (Low)","0.25","0.50","0.75","1 (High)"],
                    len=0.6,
                    # Colorbar title and tick text: explicit dark colour
                    # so it reads on both light (white bg) and dark map tiles
                    title_font=dict(color="#1a1a1a", size=12),
                    tickfont=dict(color="#1a1a1a", size=11),
                    bgcolor="rgba(255,255,255,0.88)",
                    bordercolor="rgba(0,0,0,0.15)",
                    borderwidth=1,
                ),
            )
            st.plotly_chart(fig, use_container_width=True)
            st.caption(
                "Moran's I = 0.804 (z = 8.31, p < 0.001) — strong positive spatial autocorrelation. "
                "Southwest cluster (Khulna + Barisal) = highest vulnerability. "
                "Hover over any district for details."
            )

            with st.expander("📊  Full district table"):
                tbl = vuln_df_map[
                    ["district_name","division_name","vulnerability_index",
                     "vulnerability_tier","n_poor_ndvi_years","mean_ndvi_boro"]
                ].sort_values("vulnerability_index", ascending=False).reset_index(drop=True)
                tbl.index += 1
                if sel_tier_map:
                    tbl = tbl[tbl["vulnerability_tier"].isin(sel_tier_map)]
                st.dataframe(
                    tbl.style
                    .background_gradient(subset=["vulnerability_index"],
                                         cmap="RdYlGn_r", vmin=0, vmax=1)
                    .format({"vulnerability_index":"{:.3f}","mean_ndvi_boro":"{:.3f}"}),
                    use_container_width=True,
                )
        else:
            st.error("Shapefile not found — run the pipeline notebook first.")

    with tab2:
        if lisa_df_map is not None:
            map_style2 = st.selectbox("Map style",
                ["carto-positron","carto-darkmatter","open-street-map"], key="lisa_style")

            LISA_COLORS = {
                "HH (hotspot)":   "#993C1D",
                "LL (coldspot)":  "#0F6E56",
                "HL (outlier)":   "#EF9F27",
                "LH (outlier)":   "#85B7EB",
                "Not significant":"#C8C5BC",
            }
            hov = {c:True for c in ["division_name","vulnerability_index","lisa_p"]
                   if c in lisa_df_map.columns}
            if "vulnerability_index" in hov: hov["vulnerability_index"]=":.3f"
            if "lisa_p" in hov:             hov["lisa_p"]=":.3f"

            fig = px.choropleth_mapbox(
                lisa_df_map, geojson=lisa_geojson, locations=lisa_df_map.index,
                color="cluster", color_discrete_map=LISA_COLORS,
                hover_name="district_name", hover_data=hov,
                mapbox_style=map_style2, zoom=6,
                center={"lat":23.7,"lon":90.4}, opacity=0.82,
                labels={"cluster":"LISA Cluster"},
                category_orders={"cluster":list(LISA_COLORS.keys())},
            )
            # Legend: always readable — use explicit white bg with dark text
            fig.update_layout(
                height=620, margin=dict(t=0,b=0,l=0,r=0),
                legend=dict(
                    title=dict(text="LISA Cluster", font=dict(color="#1a1a1a",size=12)),
                    font=dict(color="#1a1a1a", size=12),
                    bgcolor="rgba(255,255,255,0.90)",
                    bordercolor="rgba(0,0,0,0.15)",
                    borderwidth=1,
                ),
            )
            st.plotly_chart(fig, use_container_width=True)

            lc1,lc2,lc3,lc4 = st.columns(4)
            cc = lisa_df_map["cluster"].value_counts()
            for col, lbl in zip([lc1,lc2,lc3,lc4],
                                ["HH (hotspot)","LL (coldspot)",
                                 "HL (outlier)","LH (outlier)"]):
                col.metric(lbl, cc.get(lbl,0))

            st.caption(
                "HH hotspots: 14 districts — Khulna + Barisal divisions "
                "(river flooding + coastal tidal inundation convergence). "
                "LL coldspots: 10 districts — Mymensingh, Sylhet haor region, "
                "Chattogram hill tracts."
            )
        else:
            st.warning("LISA results not found — run the spatial analysis notebook first.")


# ══════════════════════════════════════════════════════════════════
# PAGE 4 — FLOOD & CLIMATE
# ══════════════════════════════════════════════════════════════════
elif page == "Flood & Climate":
    st.title("Flood Exposure and Climate Trends")

    with st.expander("🔽  Filters", expanded=True):
        ff1, ff2 = st.columns(2)
        with ff1:
            sel_div_fc = st.multiselect("Division", ALL_DIVS, default=[], key="fc_div",
                                        placeholder="All divisions")
        with ff2:
            yr_fc = st.slider("Year range", YEAR_MIN, YEAR_MAX,
                               (YEAR_MIN, YEAR_MAX), key="fc_yr")

    dfc = df_panel.copy()
    if sel_div_fc:
        dfc = dfc[dfc["division_name"].isin(sel_div_fc)]
    dfc = dfc[(dfc["year"]>=yr_fc[0]) & (dfc["year"]<=yr_fc[1])]

    col1, col2 = st.columns(2)

    with col1:
        st.subheader("Mean flood fraction by division")
        if "flood_fraction" in dfc.columns:
            df_div = (dfc.dropna(subset=["flood_fraction"])
                      .groupby("division_name")["flood_fraction"].mean()
                      .sort_values().reset_index())
            df_div.columns = ["Division","Mean flood fraction"]
            fig = px.bar(df_div, x="Mean flood fraction", y="Division",
                         orientation="h", color="Mean flood fraction",
                         color_continuous_scale="Blues",
                         text=df_div["Mean flood fraction"].map("{:.3f}".format))
            fig.update_traces(textposition="outside")
            fig.update_layout(height=360, margin=dict(t=10,b=10,l=10,r=10),
                               showlegend=False, coloraxis_showscale=False,
                               **PLOT_BG, xaxis_gridcolor=GRID)
            st.plotly_chart(fig, use_container_width=True)

    with col2:
        st.subheader("Districts with major floods per year")
        if "is_major_flood_year" in dfc.columns:
            fy = dfc.groupby("year")["is_major_flood_year"].sum().reset_index()
            fy.columns = ["Year","Districts"]
            fig = px.bar(fy, x="Year", y="Districts",
                         color="Districts", color_continuous_scale="YlOrRd",
                         text="Districts")
            fig.update_traces(textposition="outside")
            fig.update_layout(height=360, margin=dict(t=10,b=10,l=10,r=10),
                               showlegend=False, coloraxis_showscale=False,
                               **PLOT_BG, yaxis_gridcolor=GRID)
            st.plotly_chart(fig, use_container_width=True)

    st.divider()
    st.subheader("NDVI anomaly heatmap — all districts × years")
    st.caption(
        "Red = below-average crop health for that district (proxy for yield stress). "
        "Major flood years (2004, 2007, 2017) appear as horizontal red bands."
    )
    pivot = dfc.pivot_table(index="district_name", columns="year",
                             values="ndvi_anomaly", aggfunc="mean")
    if not pivot.empty:
        fig = px.imshow(pivot, color_continuous_scale="RdYlGn",
                        color_continuous_midpoint=0, aspect="auto",
                        labels={"color":"NDVI anomaly","x":"Year","y":"District"})
        fig.update_coloraxes(
            colorbar_title="NDVI<br>anomaly",
            colorbar_title_font=dict(color="#1a1a1a", size=12),
            colorbar_tickfont=dict(color="#1a1a1a", size=11),
            colorbar_bgcolor="rgba(255,255,255,0.88)",
            colorbar_bordercolor="rgba(0,0,0,0.15)",
            colorbar_borderwidth=1,
        )
        fig.update_layout(height=max(400, min(80*len(pivot),800)),
                          margin=dict(t=10,b=20,l=20,r=20))
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("No NDVI data for selected filters.")


# ══════════════════════════════════════════════════════════════════
# PAGE 5 — REGRESSION FINDINGS
# ══════════════════════════════════════════════════════════════════
elif page == "Regression Findings":
    st.title("Statistical Modelling Findings")
    st.markdown(
        "**Model:** Two-way fixed-effects panel regression (district + year FE) · "
        "Standard errors clustered by district · "
        "**Extension:** Spatial Lag Model (GM-Lag) accounting for spatial autocorrelation "
        "(Moran's I = 0.804 on residuals)."
    )

    # ── Panel regression results ───────────────────────────────────
    reg_path  = ROOT / "outputs/analysis/regression_results_table.csv"
    coef_path = ROOT / "outputs/analysis/fig3_regression_coefficients.png"

    if reg_path.exists():
        df_reg = pd.read_csv(reg_path)
        st.subheader("Fixed-Effects Panel Regression — three-model comparison")
        st.dataframe(df_reg, use_container_width=True)
        st.caption("\\*\\*\\* p<0.01  \\*\\* p<0.05  \\* p<0.1 · SE clustered by district · "
                   "Within-R² = 0.025 reflects that district + year FE absorb the large majority "
                   "of variance; climate regressors explain within-district deviations. "
                   "F-statistic = 6.76 (p < 0.001) confirms joint significance.")
    else:
        st.warning("Regression results CSV not found — run the econometric notebook first.")

    if coef_path.exists():
        st.divider()
        st.subheader("Model 2 — Coefficient plot with 95% CI")
        st.image(str(coef_path), use_container_width=True)

    # ── Spatial lag model results ──────────────────────────────────
    st.divider()
    st.subheader("Spatial Lag Model (SLM) — accounting for geographic spillovers")
    st.markdown(
        "Following Moran's I = 0.804, a Spatial Lag Model was estimated using "
        "Queen contiguity weights. The spatial autoregressive coefficient ρ captures "
        "co-movement between neighbouring districts driven by shared agro-ecological "
        "conditions and correlated climate shocks."
    )
    if df_slm is not None:
        # Separate the W_yield row for the rho metric
        rho_row = df_slm[df_slm["variable"].str.contains("W_yield", na=False)]
        coef_rows = df_slm[~df_slm["variable"].str.contains("W_yield|CONSTANT", na=False)]

        if not rho_row.empty:
            rho_val = rho_row["coefficient"].values[0]
            rho_p   = rho_row["p_value"].values[0]
            sig     = "***" if rho_p < 0.01 else "**" if rho_p < 0.05 else "*" if rho_p < 0.10 else ""
            sm1, sm2, sm3 = st.columns(3)
            sm1.metric("ρ (spatial autoregressive)", f"{rho_val:.4f}{sig}")
            sm2.metric("p-value (ρ)", f"{rho_p:.4f}")
            sm3.metric("Interpretation",
                       "Significant clustering" if rho_p < 0.05 else "Weak spatial dependence")

        if not coef_rows.empty:
            fig = go.Figure()
            y_labels = coef_rows["variable"].tolist()
            coefs    = coef_rows["coefficient"].tolist()
            ses      = coef_rows["std_error"].tolist()
            colors   = ["#3B6D11" if c > 0 else "#993C1D" for c in coefs]

            fig.add_trace(go.Scatter(
                x=coefs, y=y_labels, mode="markers",
                marker=dict(color=colors, size=10, symbol="circle"),
                error_x=dict(type="data", array=[1.96*s for s in ses],
                             color="rgba(120,120,120,0.6)", thickness=1.5, width=6),
                name="SLM coefficient",
            ))
            fig.add_vline(x=0, line_dash="dash", line_color="rgba(120,120,120,0.5)")
            fig.update_layout(
                title="Spatial Lag Model — coefficient plot (95% CI)",
                xaxis_title="Coefficient (effect on Boro yield, MT/ha)",
                height=280, margin=dict(t=40,b=20,l=20,r=20),
                **PLOT_BG,
                xaxis=dict(showgrid=True, gridcolor=GRID),
                yaxis=dict(showgrid=False),
                # Legend not needed for single-series plot — hide it
                showlegend=False,
            )
            st.plotly_chart(fig, use_container_width=True)
            st.caption(
                "SLM coefficients are comparable to the FE panel estimates. "
                "Qualitative consistency across both models strengthens the causal interpretation."
            )
    else:
        st.info(
            "Spatial lag model results not found. "
            "Run Section 4 of `03_Econometric_Modelling.ipynb` to generate "
            "`outputs/analysis/spatial_lag_model_results.csv`."
        )

    # ── Interpretation table ───────────────────────────────────────
    st.divider()
    st.subheader("Variable interpretation")
    interp = {
        "Variable":       ["NDVI anomaly","Precipitation anomaly",
                           "Temperature anomaly","Flood fraction (contemporaneous)",
                           "Flood fraction (lagged)"],
        "Direction":      ["⬆ Positive***","⬆ Positive (ns)",
                           "⬆ Positive**","⬇ Negative (ns)","⬇ Negative (test)"],
        "Mechanism":      [
            "Higher crop health → higher yield (dominant predictor)",
            "More Boro-season rain → better yield; ns because Boro is irrigated",
            "Cold-stress relief in Jan–May cool window (below spikelet sterility threshold)",
            "Aman flooding reduces next Boro yield via soil waterlogging; imprecisely estimated",
            "One-season lag tests Aman→Boro temporal mechanism (see Model 3)",
        ],
    }
    st.dataframe(pd.DataFrame(interp), use_container_width=True, hide_index=True)

    # ── NDVI sign reversal note ────────────────────────────────────
    st.divider()
    with st.expander("📌  Note on BBS-only robustness check"):
        st.markdown("""
The NDVI coefficient diverges between the full sample (+1.83) and the BBS-actual subsample
(2012–2022). Two mechanisms explain this:

1. **Circularity:** NDVI is used both to construct the pre-2012 yield proxy (dependent variable)
   and as a regressor — mechanically inflating the full-sample coefficient.
2. **Multicollinearity in the short window:** The 2012–2022 period coincides with a strong
   upward NDVI trend (Mann-Kendall τ = +0.697, p < 0.001) driven by HYV variety diffusion.
   In a short panel with year fixed effects, this trend is largely absorbed by the year dummies,
   leaving the within-district NDVI variation to carry a negative sign due to
   overfitting of the trend component.

**Conclusion:** The BBS-only coefficient should not be interpreted as a genuine negative
NDVI-yield effect. The full-sample estimate (+1.83) overstates the effect; the BBS-only
estimate is confounded by the trend-FE interaction. Both motivate careful interpretation
and confirm the value of extending BBS data backwards via a cleaner proxy approach.
        """)

    st.info(
        "Southwest HH-hotspot districts (Khulna, Barisal divisions) show substantially "
        "stronger negative flood coefficients in the tier-stratified analysis "
        "— consistent with their structural BARC flood risk classification."
    )


# ══════════════════════════════════════════════════════════════════
# PAGE 6 — ABOUT
# ══════════════════════════════════════════════════════════════════
elif page == "About":
    st.title("About this project")

    st.markdown("""
### Climate-Driven Vulnerability of Boro Rice Yield Across Bangladesh's 64 Districts (2001–2022)

**Research question:**
To what extent do satellite-derived flood inundation extent and NDVI-based crop health,
combined with rainfall variability, explain district-level Boro rice yield disparities
across Bangladesh from 2001 to 2022, and which districts are most vulnerable to
compounding climate stress?
""")

    st.divider()
    col1, col2 = st.columns(2)

    with col1:
        st.markdown("#### Data sources")
        sources = {
            "Source":    ["BBS Agricultural Yearbooks","MODIS MOD13Q1 v061 (GEE)",
                          "ERA5 Monthly Means (CDS)","JRC Global Surface Water (GEE)",
                          "BARC Flood Hazard / HDX","GADM v4.1","FAO FAOSTAT"],
            "Variable":  ["District Boro yield","NDVI crop health",
                          "Temperature & precipitation","Annual flood inundation fraction",
                          "Static flood risk zones","District boundaries",
                          "National rice production"],
            "Period":    ["2012–2022","2001–2023","2001–2023","2001–2021",
                          "Static","Static","2001–2023"],
        }
        st.dataframe(pd.DataFrame(sources), use_container_width=True, hide_index=True)

    with col2:
        st.markdown("#### Key findings")
        st.markdown("""
- **Moran's I = 0.804** (z = 8.31, p < 0.001) — strong geographic clustering
- **14 HH hotspot districts** — southwest delta (Khulna + Barisal)
- **10 LL coldspot districts** — Mymensingh, Sylhet haor, Chattogram hill tracts
- **NDVI anomaly** dominant predictor (β = +1.83 MT/ha, p < 0.01)
- **Temperature anomaly** positive (β = +0.097 MT/ha, p < 0.05) — cold-stress relief
- **Boro yield trend:** +0.015 MT/ha/year (Mann-Kendall, p < 0.001) — technology-driven
        """)
        st.markdown("#### Methodology")
        st.markdown("""
- Data engineering: Bronze → Silver → Gold medallion pipeline (Python · DuckDB)
- Spatial: Queen contiguity weights · Global & Local Moran's I (PySAL/esda)
- Econometric: Two-way FE panel regression (linearmodels) · SE clustered by district
- Spatial regression: GM-Lag Spatial Lag Model + LM diagnostics (spreg)
- Trend: Mann-Kendall test + Sen's slope (pymannkendall)
- Scenario: IPCC AR6 illustrative scenarios (SSP2-4.5 / SSP5-8.5)
        """)

    st.divider()
    st.markdown(
        "**GitHub:** [github.com/TasMumu](https://github.com/TasMumu?tab=repositories) "
        "| **Contact:** tasnimmumu414@gmail.com"
    )